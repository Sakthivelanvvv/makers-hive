...
function PurchasePage() {
  ...
  const create = useMutation({
    mutationFn: async (v: { vendor: string; vendor_address: string; amount: number }) => {
      const vendorSchema = requiredStringSchema("Vendor");
      const addressSchema = requiredStringSchema("Vendor address");
      const amountSchema = positivePriceSchema("Amount");
      const parsedVendor = vendorSchema.safeParse(v.vendor);
      const parsedAddress = addressSchema.safeParse(v.vendor_address);
      const parsedAmount = amountSchema.safeParse(v.amount);
      if (!parsedVendor.success) throw new Error(parsedVendor.error.issues[0]?.message ?? "Invalid vendor.");
              ...
              {orders.map((o) => (
                <TableRow key={o.id}>
                  ...
                  <TableCell>
                    <Actions
                      status={o.status}
                      onApprove={() => setStatus.mutate({ id: o.id, status: "approved" })}
...
function Actions({
  ...
  if (status === "draft") {
    buttons.push(
      <Button key="approve" size="sm" onClick={onApprove}>
        Submit Approval
      </Button>,
    );
  }
  ...
  if (status === "approved" || status === "confirmed") {
    buttons.push(
      <Button key="confirm" size="sm" onClick={onConfirm}>
        Confirm
      </Button>,
    );
  }
  if (["confirmed", "partially_received"].includes(status)) {
    buttons.push(
      <Button key="r" size="sm" variant="secondary" onClick={onReceive}>
        Receive
      </Button>,
    );
    buttons.push(
...
function NewPoForm({
  ...
  onSubmit: (v: { vendor: string; vendor_address: string; amount: number }) => void;
  pending: boolean;
}) {
    ...
    <DialogContent>
      ...
      >
        <div className="space-y-2">
          <Label>Vendor</Label>
          <Input required value={v.vendor} onChange={(e) => setV({ ...v, vendor: e.target.value })} />
        </div>
        <div className="space-y-2">
          <Label>Address</Label>
          <Input value={v.vendor_address} onChange={(e) => setV({ ...v, vendor_address: e.target.value })} />
        </div>
        <div className="space-y-2">
